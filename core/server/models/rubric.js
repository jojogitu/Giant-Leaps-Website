...
Rubric = ghostBookshelf.Model.extend({

    tableName: 'rubrics',

    emitChange: function emitChange(event) {
        events.emit('rubric' + '.' + event, this);
    },
    ...
    saving: function saving(newPage, attr, options) {
        ...
        if (this.hasChanged('slug') || !this.get('slug')) {
            ...
            return ghostBookshelf.Model.generateSlug(Rubric, this.get('slug') || this.get('name'),
    ...
    findOne: function findOne(data, options) {
        ...
        var rubric = this.forge(data);

        // Add related objects
        options.withRelated = _.union(options.withRelated, options.include);

        return rubric.fetch(options);
    },
...
Rubrics = ghostBookshelf.Collection.extend({
    model: Rubric
});

module.exports = {
    Rubric: ghostBookshelf.model('Rubric', Rubric),
    Rubrics: ghostBookshelf.collection('Rubrics', Rubrics)
};
